...
export const Account = () => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>, section?: 'address') => {
    if (!formData) return;
  ...
  if (!user || !formData) return <div className="p-20 text-center">Loading...</div>;
    ...
    <div className="container mx-auto px-4 py-10 max-w-7xl">
       ...
       <div className="flex flex-col lg:flex-row gap-8">
          ...
          <aside className="lg:w-1/4">
             <div className="bg-accent/30 p-4 rounded-xl shadow-sm">
                ...
                <nav className="space-y-2">
                   ...
                   <Link to="/orders" className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-white transition-colors font-medium text-dark">
                      <span className="material-symbols-outlined">receipt_long</span> Order History
                   </Link>
                   <Link to="/settings" className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-white transition-colors font-medium text-dark">
                      <span className="material-symbols-outlined">settings</span> Settings
                   </Link>
                   <Link to="/favorites" className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-white transition-colors font-medium text-dark">
                      <span className="material-symbols-outlined">favorite</span> Favorites
                   </Link>
                </nav>
                <div className="mt-8 pt-4 border-t border-gray-200">
                   <button onClick={handleLogout} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-white transition-colors text-red-500 font-medium">
          ...
          <div className="flex-1 bg-white p-8 rounded-xl shadow-sm border border-secondary/10">
             <h2 className="text-2xl font-bold mb-6 pb-2 border-b border-gray-100">Profile Information</h2>
             <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                   <label className="block">
                      <span className="text-sm font-medium mb-1 block">First Name</span>
                      <input 
                        type="text" 
                        name="firstName"
                   ...
                   <label className="block">
                      <span className="text-sm font-medium mb-1 block">Last Name</span>
                      <input 
                        type="text" 
                        name="lastName"
                ...
                <h2 className="text-2xl font-bold mt-8 mb-6 pb-2 border-b border-gray-100">Shipping Address</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
